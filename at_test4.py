...
ALPHA = 0.99
POWERS = np.arange(0,SMALL_AVERAGE_LENGTH-2,1)
FACTORS = np.power(np.ones(SMALL_AVERAGE_LENGTH-2)*ALPHA,POWERS)
...
class AutoTrader(BaseAutoTrader):
    ...
    def pop_order(self):
        ...
        return [self.time_order_queue[self.queue_head], self.id_order_queue[self.queue_head],
                self.side_order_queue[self.queue_head], \
    ...
    def trend_spotter(self):
        ...
        self.short_term_grad = (self.mid_prices[-1] - self.mid_prices[-SMALL_AVERAGE_LENGTH]) / (
                SMALL_AVERAGE_LENGTH * TICK_INTERVAL)
    ...
    def risk_metrics(self):
        prices = np.array(self.mid_prices[-1:-SMALL_AVERAGE_LENGTH:-1])
        if (prices != 0).all():
            last_ret = np.log(prices[0] / prices[1])
            lmbda = 0.97  # standard for monthly returns. 1 is equivalent to historical avg, lower value smoothes more
            vol = lmbda * self.vol + (1 - lmbda) * last_ret ** 2
            self.vol = vol
            time_adjustment = 93600  # 4 trades per second —> observations for 6,5 hours open stock exchange

            time = self.current_time
            #with open('risk_metrics_data.txt', 'a') as f:
            #    f.write('\n' + str(time) + ',' + str(self.vol))
                # print('writing to RiskMetrics data')

    def exp_fact(self):
        prices = np.array(self.mid_prices[-1:-SMALL_AVERAGE_LENGTH:-1])
        if (prices!=0).all():
            returns = np.diff(prices) / prices[0:-1]
            sig = -np.dot(FACTORS,returns)*1e6
            # if sig>EWMA_THRESHOLD:
            #     # print('POSITIVE TREND')
            #     self.hedge_delay = -1
            # elif sig<-EWMA_THRESHOLD:
    ...
    def on_order_book_update_message(self, instrument: int, sequence_number: int, ask_prices: List[int],
        ...
        if instrument == Instrument.FUTURE:
            ...
            exp_sig = int(self.exp_fact() / EWMA_THRESHOLD) * TICK_SIZE_IN_CENTS
            self.risk_metrics()
            S = 6
            if self.vol < 2e-7:
                S = 4
            ...
            spread = S * TICK_SIZE_IN_CENTS

            new_bid_price = mid - spread // 2 + price_adjustment + exp_sig if bid_prices[0] != 0 else 0
            new_ask_price = mid + spread // 2 + price_adjustment + exp_sig if ask_prices[0] != 0 else 0
            ...
            if self.bid_id == 0 and new_bid_price != 0 and self.position < 1.0 * (POSITION_LIMIT - 1.6 * LOT_SIZE):
                self.arb_ticker = 0
            ...
            if self.ask_id == 0 and new_ask_price != 0 and self.position > -1.0 * (POSITION_LIMIT - 1.6 * LOT_SIZE):
                self.arb_ticker = 0
        ...
        if instrument == Instrument.ETF:
            ...
            if (self.last_bid_fut != 0) and (ask_prices[0] != 0):
                delta_b = (self.last_bid_fut - ask_prices[0]) // TICK_SIZE_IN_CENTS

            # print(delta_a,delta_b)
            if delta_a > 0 and self.position > -0.8 * POSITION_LIMIT and self.arb_ticker < 2:
                self.arb_ticker += 1
            ...
            if delta_b > 0 and self.position < 0.8 * POSITION_LIMIT and self.arb_ticker < 2:
                self.arb_ticker += 1
    ...
    def on_order_filled_message(self, client_order_id: int, price: int, volume: int) -> None:
        ...
        if client_order_id in self.bids:
            ...
            if self.hedge_delay == -1 and client_order_id != self.bid_id_arb:
                ...
                # print('did a hedge with volume', volume)
            ...
            if self.hedge_delay == +1 and client_order_id != self.ask_id_arb:
                ...
                # print('did a hedge on bid side with volume ', volume)
