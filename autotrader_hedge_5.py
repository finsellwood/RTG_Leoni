...
# to check if using available prices for future rather than sold prices of etf
# aim to develop code to separately store bid and ask hedges, dump all of one type when hedge type switches
LOT_SIZE = 10
POSITION_LIMIT = 100
...
class AutoTrader(BaseAutoTrader):
    ...
    def __init__(self, loop: asyncio.AbstractEventLoop, team_name: str, secret: str):
        ...
        # self.a_id_order_queue = [0] * self.queue_length #stores ids of orders
        self.a_vol_order_queue = [0] * self.queue_length # stores volume of orders to be executed
        # self.a_side_order_queue = [0] * self.queue_length # stores buy/ask side of orders to be execd
        self.a_time_order_queue = [0] * self.queue_length # stores the time for orders to be execd
        self.a_price_order_queue = [0] * self.queue_length # stores the price of orders to be execd

        self.a_queue_head = self.a_queue_tail = 0

        # bid que variables
        # self.b_id_order_queue = [0] * self.queue_length #stores ids of orders
        self.b_vol_order_queue = [0] * self.queue_length # stores volume of orders to be executed
        # self.b_side_order_queue = [0] * self.queue_length # stores buy/ask side of orders to be execd
        self.b_time_order_queue = [0] * self.queue_length # stores the time for orders to be execd
        self.b_price_order_queue = [0] * self.queue_length # stores the price of orders to be execd

        self.b_queue_head = self.b_queue_tail = 0

        # debug variables
        self.no_bid_delayed = self.no_ask_delayed = 0

    def delay_hedge_order(self, execution_time:int, buy_or_ask:bool, order_price:int, order_vol:int):
        # true == buy, false == ask
        # push an order to a queue of orders, to be executed at a given counter value (execution time)
        print(self.b_queue_tail, self.b_queue_head)
        if buy_or_ask == True:
            self.b_queue_tail = (self.b_queue_tail + 1) % self.queue_length
            # self.id_order_queue[self.queue_tail] = order_id
            self.b_vol_order_queue[self.b_queue_tail] = order_vol
            # self.side_order_queue[self.queue_tail] = buy_or_ask
            self.b_price_order_queue[self.b_queue_tail] = order_price
            self.b_time_order_queue[self.b_queue_tail] = execution_time
        if buy_or_ask == False:
            self.a_queue_tail = (self.a_queue_tail + 1) % self.queue_length
            # self.id_order_queue[self.queue_tail] = order_id
            self.a_vol_order_queue[self.a_queue_tail] = order_vol
            # self.side_order_queue[self.queue_tail] = buy_or_ask
            self.a_price_order_queue[self.a_queue_tail] = order_price
            self.a_time_order_queue[self.a_queue_tail] = execution_time
    

    def a_pop_order(self):
        self.a_queue_head = (self.a_queue_head + 1) % self.queue_length
        return [self.a_time_order_queue[self.a_queue_head], self.a_price_order_queue[self.a_queue_head], self.a_vol_order_queue[self.a_queue_head]]
    
    def b_pop_order(self):
        self.b_queue_head = (self.b_queue_head + 1) % self.queue_length
        return [self.b_time_order_queue[self.b_queue_head], self.b_price_order_queue[self.b_queue_head], self.b_vol_order_queue[self.b_queue_head]]
    
    def a_peek_order(self):
        ...
        return self.a_time_order_queue[self.a_queue_head]
    def b_peek_order(self):

        # returns the execution time of the next order in the queue
        # return [self.time_order_queue[self.queue_head], self.id_order_queue[self.queue_head], self.side_order_queue[self.queue_head], self.vol_order_queue[self.queue_head]]
        return self.b_time_order_queue[self.b_queue_head]
    

    def a_check_empty_hedge_queue(self):
        return self.a_queue_head == self.a_queue_tail
    def b_check_empty_hedge_queue(self):
        return self.b_queue_head == self.b_queue_tail
    ...
    def trend_spotter(self):
        ...
        if len(self.mid_prices) >= MEDIUM_AVERAGE_LENGTH:
            self.med_term_grad = (self.mid_prices[-1] - self.mid_prices[-MEDIUM_AVERAGE_LENGTH]) / (MEDIUM_AVERAGE_LENGTH*TICK_INTERVAL)
    ...
    def execute_delayed_hedge(self, side):
        # so that i don't have to write this each time - executes next queued hedge order on either queue
        if side == Side.ASK:
            order_info = self.a_pop_order()
            self.send_hedge_order(next(self.order_ids), Side.ASK, order_info[1], order_info[2])
            print('executed delayed hedge on ASK side with volume', order_info[2], 'at time ', self.current_time)
            self.no_ask_delayed -=1

        if side == Side.BID:
            order_info = self.b_pop_order()
            self.send_hedge_order(next(self.order_ids), Side.BID, order_info[1], order_info[2])
            print('executed delayed hedge on BID side with volume', order_info[2], 'at time ', self.current_time)
    ...
    def on_order_book_update_message(self, instrument: int, sequence_number: int, ask_prices: List[int],
        ...
        a_next_order_exec_time = self.a_peek_order()
        b_next_order_exec_time = self.b_peek_order()


        if not self.a_check_empty_hedge_queue() and a_next_order_exec_time <= self.current_time:
            self.execute_delayed_hedge(Side.ASK)

        if not self.b_check_empty_hedge_queue() and b_next_order_exec_time <= self.current_time:
            self.execute_delayed_hedge(Side.BID)
    ...
    def on_order_filled_message(self, client_order_id: int, price: int, volume: int) -> None:
        ...
        if client_order_id in self.bids:
            ...
            if self.hedge_delay == -1:
                # delay the hedge
                self.delay_hedge_order(self.current_time+HEDGE_DELAY, Side.ASK, MIN_BID_NEAREST_TICK, volume)
                # print('delayed ASK hedge order with volume', volume, 'at time ', self.current_time)
                self.no_ask_delayed +=1
                print( self.b_check_empty_hedge_queue(), 'bid')
                while self.b_check_empty_hedge_queue() == False:
                    self.execute_delayed_hedge(Side.BID)
                    # print('emptying BID queue')
            ...
            if self.hedge_delay == +1:
                ...
                self.delay_hedge_order(self.current_time+HEDGE_DELAY , Side.BID, MAX_ASK_NEAREST_TICK, volume)
                # print('delayed BID hedge order with volume', volume, 'at time ', self.current_time)
                self.no_bid_delayed +=1
                print(self.a_check_empty_hedge_queue(), 'ask')
                while self.a_check_empty_hedge_queue() == False:
                    self.execute_delayed_hedge(Side.ASK)
                    # print('emptying ASK queue')
            else:
