...
class AutoTrader(BaseAutoTrader):
    ...
    def trend_spotter(self):
        ...
        self.ewma = (ALPHA * self.mid_prices[-1]) + (1-ALPHA) * self.ewma
    ...
    def on_order_book_update_message(self, instrument: int, sequence_number: int, ask_prices: List[int],
        ...
        if len(self.arb_bids) > 2:
            self.send_cancel_order(min(self.arb_bids))
        if len(self.arb_asks) > 2:
            self.send_cancel_order(min(self.arb_asks))
        if len(self.bids) > 2:
    ...
    def on_order_filled_message(self, client_order_id: int, price: int, volume: int) -> None:
        ...
        self.logger.info("received order filled for order %d with price %d and volume %d", client_order_id,
                         price, volume)
        if client_order_id in self.bids or client_order_id in self.arb_bids:
        ...
        elif client_order_id in self.asks or client_order_id in self.arb_asks:
            self.position -= volume
            if self.hedge_delay == +1:
