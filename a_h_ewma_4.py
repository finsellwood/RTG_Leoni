...
class AutoTrader(BaseAutoTrader):
    ...
    def change_detect_simple(self):
        if self.short_term_grad > 5:
            self.hedge_delay = -1
        elif self.short_term_grad < -5:
            self.hedge_delay = +1
        else:
            self.hedge_delay = 0
    ...
    def change_detect_grad_plus_breakout(self):
        frac_price_over_average = (self.mid_prices[-1] - self.ew_mid_prices[-1])/self.ew_mid_prices[-1]
        price_over_average = (self.mid_prices[-1] - self.ew_mid_prices[-1])
        gradient = 0
        if len(self.ew_mid_prices) >= 2:
            gradient = (self.ew_mid_prices[-1] - self.ew_mid_prices[-2])/2
        # print(frac_price_over_average, gradient)
        # print(price_over_average)
        # if frac_price_over_average > 0.1 and gradient > 0:
        #     # if we're 10% over the average price, and the gradient is positive, delay the (???) hedges
        #     self.hedge_delay = -1
        # elif frac_price_over_average < -0.1 and gradient < 0:
        #     self.hedge_delay = +1
        # else:
        #     self.hedge_delay = 0
        if price_over_average > 1 * TICK_SIZE_IN_CENTS and gradient > 0:
            # if we're 10% over the average price, and the gradient is positive, delay the (???) hedges
            self.hedge_delay = -1
            print(price_over_average, 'over average, delay =-1')
        elif price_over_average < -1 * TICK_SIZE_IN_CENTS and gradient < 0:
            self.hedge_delay = +1
            print(price_over_average, 'under average, delay +1')
        else:
            self.hedge_delay = 0
            print(price_over_average, 'no hedge delay')


    def trend_spotter(self):
        # try and notice positive or negative trend (currently using futures data)
        gradient = (self.mid_prices[-1] - self.mid_prices[-2])/2
        self.ewma = (ALPHA * gradient) + (1-ALPHA) * self.ewma
    ...
    def trend_spotter_ewma(self):
        # try and notice positive or negative trend (currently using futures data)
        if len(self.ew_mid_prices) >= SMALL_AVERAGE_LENGTH:
            self.short_term_grad = (self.ew_mid_prices[-1] - self.ew_mid_prices[-SMALL_AVERAGE_LENGTH]) / (SMALL_AVERAGE_LENGTH)
            print(self.short_term_grad)
        if len(self.ew_mid_prices) >= MEDIUM_AVERAGE_LENGTH:
            self.med_term_grad = (self.ew_mid_prices[-1] - self.ew_mid_prices[-MEDIUM_AVERAGE_LENGTH]) / (MEDIUM_AVERAGE_LENGTH)
            # print(self.short_term_grad, self.med_term_grad)
            # if len(self.mid_prices) >=100:
            #     self.long_term_grad = (self.mid_prices[-1] - self.mid_prices[-100]) / (5*TICK_INTERVAL)
        self.change_detect_simple()
    ...
    def trend_spotter_ewma_2(self):
        if len(self.ew_mid_prices) >= 3:
            grad_now = (self.ew_mid_prices[-1] - self.ew_mid_prices[-2])/2
            grad_before = (self.ew_mid_prices[-2] - self.ew_mid_prices[-3])/2
            if grad_before !=0:
                self.delta = (grad_now - grad_before)/grad_now
        # if len(self.ew_mid_prices) >= 10:
        #     self.delta = (self.ew_mid_prices[-1] - 2 * self.ew_mid_prices[-2] + self.ew_mid_prices[-2])/(self.ew_mid_prices[-2] - self.ew_mid_prices[-3])
        # print(self.delta)
        # self.change_detect_delta()
        self.change_detect_grad_plus_breakout()
    ...
    def calc_midpoints(self, bid_prices, ask_prices):
        ...
        if not self.ew_mid_prices:
            self.ew_mid_prices.append(middle)
        else:
            self.ew_mid_prices.append((ALPHA * middle) + (1-ALPHA) * self.ew_mid_prices[-1])
        self.trend_spotter_ewma_2()
    ...
    def on_order_book_update_message(self, instrument: int, sequence_number: int, ask_prices: List[int],
        ...
        if not self.check_empty_hedge_queue() and next_order_exec_time <= self.current_time:
            ...
            # print(MODEL_NAME, 'executed delayed hedge on side ', order_info[2], 'with volume', order_info[4], 'at time ', self.current_time)
    ...
    def on_order_filled_message(self, client_order_id: int, price: int, volume: int) -> None:
        ...
        if client_order_id in self.bids:
            ...
            # print(MODEL_NAME, 'position is ', self.position)
